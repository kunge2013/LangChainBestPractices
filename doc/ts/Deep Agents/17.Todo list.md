...
Show the todo list only when `todos.length > 0`. Before the agent creates its plan, there’s nothing to display. Showing an empty component wastes space.
...
[[官方源码]](source_code/deep-agent-todo-list.zip)
