...
/**
 * Normalize a path without depending on node:path, which may behave
 * unexpectedly in some deployment runtimes. Handles decoding,
 * collapsing slashes, and resolving `.` / `..` segments.
 */
...
/**
 * Reads the `filePath` query parameter, applies a fallback, then validates
 * that the resolved path stays under `/app` with safe characters only.
 *
 * Note: we use `filePath` instead of `path` because LangGraph Cloud injects
 * its own `path` query parameter containing the route path.
 */
...
type ListingSandbox = SandboxBackendProtocolV2 & {
  glob?: (
    pattern: string,
    searchPath?: string,
  ) => Promise<{ files?: FileInfoLike[]; error?: string | null }>;
