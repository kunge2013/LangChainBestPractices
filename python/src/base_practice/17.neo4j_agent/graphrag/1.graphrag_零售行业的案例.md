...
[人工智能助力客户体验：一个零售行业的案例](https://github.com/neo4j-product-examples/graphrag-customer-experience)


AN example application demonstrates how to implement GraphRAG for various touchpoints in the customer journey including:

1. **Discovery**: Improve click-through rate with personalized marketing
2. **Search**: Increase conversion with tailored semantic search
3. **Recommendations**: Boost average order value with customized recommendations
4. **Support**: Reduce cost to serve with well-grounded, fact-based, AI scripts
...
[![](https://github.com/neo4j-product-examples/graphrag-customer-experience/raw/main/images/app-architecture.png)](https://github.com/neo4j-product-examples/graphrag-customer-experience/blob/main/images/app-architecture.png)
...
There are two components to the graph that need to be loaded: the **Product Graph** & **Support Graph**:

### Product Graph Loading

For the product graph powering Discovery, Search, and Recommendations pages use [this notebook](https://github.com/neo4j-product-examples/graphrag-examples/blob/main/patterns-app/load-data/hm-data.ipynb) to load data. This leverages the [H&M Personalized Fashion Recommendations Dataset](https://www.kaggle.com/competitions/h-and-m-personalized-fashion-recommendations/data), a sample of real customer purchase data that includes rich information around products including names, types, descriptions, department sections, etc.

Below is the end result graph data model: [![summary](https://github.com/neo4j-product-examples/graphrag-customer-experience/raw/main/images/hm-data-model.png)](https://github.com/neo4j-product-examples/graphrag-customer-experience/blob/main/images/hm-data-model.png)
...
4. Add a graph schema. This is best practice when building graphs from unstructured text. For this example we will just define node labels to use, but in real-world examples you may need to define relationship types as well.
    1. Click the "Graph Enhancement" button then the "Get Schema From Text" button.
        2. Copy the text from the [`support-node-labels.txt`](https://github.com/neo4j-product-examples/graphrag-customer-experience/blob/main/support-node-labels.txt) file and past in the box.
           [![](https://github.com/neo4j-product-examples/graphrag-customer-experience/raw/main/images/paste-node-labels.png)](https://github.com/neo4j-product-examples/graphrag-customer-experience/blob/main/images/paste-node-labels.png) 3. check off the "text is schema description" box and press the "Analyze text to extract graph schema" button.
           [![](https://github.com/neo4j-product-examples/graphrag-customer-experience/raw/main/images/paste-node-labels-and-check.png)](https://github.com/neo4j-product-examples/graphrag-customer-experience/blob/main/images/paste-node-labels-and-check.png) 4. When complete, you should have a schema that looks like the below
           [![](https://github.com/neo4j-product-examples/graphrag-customer-experience/raw/main/images/node-labels-in-schema.png)](https://github.com/neo4j-product-examples/graphrag-customer-experience/blob/main/images/node-labels-in-schema.png) 5. Close the Graph Enhancements window
...
After completing both data loading components above, you can run the app following the directions in the [Running the App](#running-the-app) section. [![](https://github.com/neo4j-product-examples/graphrag-customer-experience/raw/main/images/support-graph-preview.png)](https://github.com/neo4j-product-examples/graphrag-customer-experience/blob/main/images/support-graph-preview.png)
...
- If you didn't increase boot disk size when creating the instance resize the disk now with the following command (often defaults to 10GB disk which will cause problems). `gcloud compute disks resize <boot disk name> --size 100` Then restart the instance for the boot disk change to take effect
